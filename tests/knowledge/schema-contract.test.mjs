...
const validator = () => {
  ...
  addFormats(ajv);
  for (const file of ['provenance.schema.json', 'assertion.schema.json', 'source.schema.json', 'entity.schema.json', 'capability-status.schema.json', 'candidate-promotion-receipt.schema.json', 'schema-validation-receipt.schema.json']) ajv.addSchema(JSON.parse(fs.readFileSync(path.join(schemaDir, file))), file);
