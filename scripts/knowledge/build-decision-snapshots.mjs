...
const sourceReviewRows = fs.existsSync(path.join(ROOT, 'evidence/source-reviews'))
  ? fs.readdirSync(path.join(ROOT, 'evidence/source-reviews')).filter(file => file.endsWith('.jsonl')).flatMap(file => fs.readFileSync(path.join(ROOT, 'evidence/source-reviews', file), 'utf8').replace(/\r\n/g, '\n').split('\n').filter(Boolean).map(JSON.parse))
  : [];
...
const buildOffer = (domain, code) => {
  ...
  const options = candidate.extracted.options
    ...
    .map((option) => {
      ...
      for (const field of ['compounding_frequency', 'interest_payment_frequency', 'day_count_convention', 'accrual_basis', 'accrual_days', 'payment_timing', 'payment_schedule_policy', 'rounding_policy']) {
        if (option[field] !== undefined && option[field] !== null) fields.push([field, option[field]]);
      }
